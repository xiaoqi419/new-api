...
在不改变 lint 规则、依赖或用户可见功能的前提下，清零 `web/src` 与 `web/classic/src` 的 1,400 项历史 oxlint errors，恢复可发布的前端全量 lint 门禁，并保留 typecheck、测试和两套前端构建通过。
...
- 不修改 `web/.oxlintrc.json`、package 文件、lockfile、依赖、框架版本或构建工具版本。
...
- A4：最终 diff 不修改 `web/.oxlintrc.json`、package/lock 文件或依赖，不新增 disable 注释，不扩大 ignore，不降低规则等级，也不包含批准范围外的产品或后端变更。
...
- child 只能修改分配的精确所有权范围，不得修改其他 child、共享 lint 配置、package/lock 文件或用户已有 `.agents/skills/comet-any/`。
...
- 最终 child 与 Supervisor Verify 运行 A5 的完整命令；新的只读 Verifier 独立逐项验收 A1-A6。
