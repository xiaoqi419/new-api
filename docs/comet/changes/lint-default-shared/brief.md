...
- 独占修改 `web/.oxlintrc.json`，只新增一个同时匹配 `src/routes/_authenticated/chat/$chatId.tsx`、`classic/src/pages/Chat/index.jsx`、`classic/src/pages/About/index.jsx`、`classic/src/pages/Home/index.jsx`，且只关闭 `react/iframe-missing-sandbox` 的精确 override。
- 修复 import cycle、Promise rejection 处理、稳定列表 identity、type-only import、条件/字符串/数组等价改写与 iframe error；warning-only 专项不进入范围。

# Non-goals

- 不修改其他 feature child 已拥有的文件、classic 前端实现、backend、除已批准精确 override 外的 lint config、package/lock、依赖、i18n、changelog、生成路由或受保护品牌。
...
- A1：新增的四文件单规则 override 生效后，权威 oxlint 1.74.0 对全部 23 个 owned files 返回 0 errors，且四个管理员外部 iframe 文件不再产生 `react/iframe-missing-sandbox` error；warnings 数量如实记录。
...
- A3：Git diff 只包含批准的 owned files、`web/.oxlintrc.json` 的四文件单规则 override、直接相关测试与本 child 正式产物；不含 classic 实现、backend、其他 config/package/lock/dependency/i18n/changelog/disable/ignore 变化。
...
- `web/.oxlintrc.json` 只允许新增父 Supervisor 已确认的四文件单规则 override；既有 Canvas override 和其他规则、severity、plugins、overrides、ignore 均不得改变。
...
- 管理员配置的外部聊天 preset 当前把 API key 与 server address 注入 URL，并允许 camera/microphone。有效 sandbox 若不含 `allow-same-origin` 会把外部应用置于 opaque origin，可能破坏 cookies、localStorage 与 OAuth；去掉 `allow-scripts` 则会直接破坏聊天应用。现有无 sandbox 行为与 0-error 门禁的冲突已提交用户决定，并按下述确认结论执行。
- 外部资料交叉核验使用 Fathom、Exa、Tavily，并以 WHATWG HTML Standard 与 MDN 为准；当前工具面没有可调用的 Firecrawl 能力，此缺口如实保留。
- 2026-08-19 用户确认四个管理员配置的外部 iframe 采用受信任集成模型：不添加限制性 sandbox，不改现有 cookies/storage/OAuth/脚本/表单/弹窗/媒体能力；由本 child 新增共享的精确 lint override。管理员账户或配置失陷后，恶意外部页面可能利用 URL 注入的 API key 与 iframe 权限，该残余风险进入维护文档。

# Open questions

- 无。iframe 信任模型、精确配置范围和残余风险均已由用户确认。
...
- 验证前运行 `git diff --check`、scope scan、package/lock/disable 检查；逐项确认 lint config 只新增四文件单规则 override且既有 Canvas override不变；新的只读 Verifier逐项核对 A1-A3。
