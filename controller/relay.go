...
func Relay(c *gin.Context, relayFormat types.RelayFormat) {
	...
	for ; retryParam.GetRetry() <= maxRetry; retryParam.IncreaseRetry() {
		...
		decision := service.DecideRelayRetry(c, newAPIError, maxRetry-retryParam.GetRetry())
...
func executeTaskSubmissionWith(
	...
	for assetRetry := 0; ; assetRetry++ {
		...
		if taskErr != nil && assetRetry == 0 && isArkPrivacyBlocked(taskErr) && autoConvertArkAssetsOnPrivacyBlock(c, relayInfo) {
			continue
		}
		break
	}
