...
func TaskArtifactContent(c *gin.Context) {
	...
	if !taskHasPluginExecution(task) {
		...
		if err := proxyLegacyVideo(c, task); err != nil {
...
func tasksToDto(tasks []*model.Task, fillUser bool, viewerRole int) []*dto.TaskDto {
	...
	for i, task := range tasks {
		...
		if task.Status == model.TaskStatusSuccess && task.Action != constant.TaskActionImagesGeneration && task.Action != constant.TaskActionImagesEdit {
			item.ResultURL = ""
