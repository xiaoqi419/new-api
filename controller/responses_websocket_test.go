...
func setupResponsesWSRequestTest(t *testing.T) (*model.User, *model.Token) {
	...
	require.NoError(t, db.AutoMigrate(&model.User{}, &model.Token{}, &model.QuotaReminderState{}))
...
func TestResponsesWSRequestRunnerPreservesTenantAndConcurrency(t *testing.T) {
	user, token := setupResponsesWSRequestTest(t)
	require.NoError(t, model.DB.Model(user).Updates(map[string]any{"agent_id": 42, "max_concurrency": 1}).Error)
	require.NoError(t, model.DB.Model(token).Update("max_concurrency", 1).Error)
	previousTimeout := common.ConcurrencyWaitTimeout
	common.ConcurrencyWaitTimeout = time.Millisecond
	t.Cleanup(func() { common.ConcurrencyWaitTimeout = previousTimeout })
	runner, _ := newResponsesWSTestRunner(t, token)
	release, ok := service.AcquireConcurrency("user", user.Id, 1)
	require.True(t, ok)
	called := false
	apiErr := runner(httptest.NewRequest(http.MethodPost, "/v1/responses", nil), "limited", func(*gin.Context) *types.NewAPIError { called = true; return nil })
	release()
	require.NotNil(t, apiErr)
	assert.Equal(t, http.StatusTooManyRequests, apiErr.StatusCode)
	assert.False(t, called)
	for _, id := range []string{"first", "second"} {
		require.Nil(t, runner(httptest.NewRequest(http.MethodPost, "/v1/responses", nil), id, func(c *gin.Context) *types.NewAPIError {
			assert.Equal(t, user.Id, c.GetInt("id"))
			assert.Equal(t, token.Id, c.GetInt("token_id"))
			assert.Equal(t, 42, common.GetContextKeyInt(c, constant.ContextKeyUserAgentId))
			tokenRelease, acquired := service.AcquireConcurrency("token", token.Id, 1)
			assert.False(t, acquired, "the active turn must hold its token slot")
			if acquired {
				tokenRelease()
			}
			return nil
...
func TestResponsesHTTPDoesNotUpgradeLegacyWebsocketSetting(t *testing.T) {
	for _, stream := range []bool{false, true} {
		t.Run(fmt.Sprint(stream), func(t *testing.T) {
			fixture := newResponsesWSBillingTest(t, `tier("request", fixed(0.002))`, func(*websocket.Conn, *http.Request) { t.Error("HTTP request upgraded to WebSocket") })
			require.NoError(t, model.DB.Model(fixture.channel).Update("setting", `{"upstream_transport":"websocket","responses_websocket_enabled":true}`).Error)
			fixture.httpUpstream = func(w http.ResponseWriter, r *http.Request) {
				assert.Equal(t, http.MethodPost, r.Method)
				assert.Empty(t, r.Header.Get("Upgrade"))
				if stream {
					w.Header().Set("Content-Type", "text/event-stream")
					_, _ = fmt.Fprint(w, "data: {\"type\":\"response.completed\",\"response\":{\"id\":\"http\",\"status\":\"completed\",\"usage\":{\"input_tokens\":1,\"output_tokens\":1,\"total_tokens\":2}}}\n\ndata: [DONE]\n\n")
				} else {
					w.Header().Set("Content-Type", "application/json")
					_, _ = fmt.Fprint(w, `{"id":"http","status":"completed","usage":{"input_tokens":1,"output_tokens":1,"total_tokens":2}}`)
				}
			}
			request, err := http.NewRequest(http.MethodPost, fixture.gatewayURL+"/v1/responses", strings.NewReader(fmt.Sprintf(`{"model":"ws-billing","input":"hello","stream":%t}`, stream)))
			require.NoError(t, err)
			request.Header.Set("Authorization", "Bearer sk-"+fixture.token.Key)
			request.Header.Set("Content-Type", "application/json")
			response, err := http.DefaultClient.Do(request)
