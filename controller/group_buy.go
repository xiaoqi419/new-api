...
func CreateGroupBuy(c *gin.Context) {
	...
	if err != nil {
		if provider == model.PaymentProviderEpay {
			failGroupBuyEpayCheckout(c.Request.Context(), userId, tradeNo)
		}
		common.ApiErrorMsg(c, err.Error())
		return
	}
...
// failGroupBuyEpayCheckout prevents a failed MAPI checkout from occupying a
// group-buy slot. Both operations are conditional, so a callback that wins the
// race cannot be reverted by checkout failure handling.
func failGroupBuyEpayCheckout(ctx context.Context, userId int, tradeNo string) {
	...
	if err := model.ReleaseGroupBuyReservation(userId, tradeNo); err != nil {
		logger.LogError(ctx, fmt.Sprintf("拼团 易支付释放预占名额失败 trade_no=%s error=%q", tradeNo, err.Error()))
	}
}
