...
func TestVideoProxyKeepsLegacyGeminiTaskCredentials(t *testing.T) {
	task := setupGenericTaskTest(t)
	allowPrivateTaskMediaTest(t)
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, "stored-task-key", r.Header.Get("x-goog-api-key"))
		assert.Equal(t, "stored-task-key", r.URL.Query().Get("key"))
		w.Header().Set("Content-Type", "video/mp4")
		_, _ = w.Write([]byte("legacy video"))
	}))
	t.Cleanup(upstream.Close)
	require.NoError(t, model.DB.Model(&model.Channel{}).Where("id = ?", task.ChannelId).Update("type", constant.ChannelTypeGemini).Error)
	task.Platform = constant.TaskPlatform(strconv.Itoa(constant.ChannelTypeGemini))
	task.PrivateData.Key = "stored-task-key"
	task.Data, _ = common.Marshal(map[string]any{"response": map[string]any{"videos": []any{map[string]any{"uri": upstream.URL}}}})
	require.NoError(t, model.DB.Save(task).Error)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Set("id", task.UserId)
	...
	VideoProxy(c)
	assert.Equal(t, http.StatusOK, recorder.Code)
	assert.Equal(t, "legacy video", recorder.Body.String())
}
