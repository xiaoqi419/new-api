		...
		t.Run(dialect.kind, func(t *testing.T) {
			...
			t.Run("media_prices_and_request_adjustments", func(t *testing.T) {
				...
				for _, name := range []string{"dall-e-2", "dall-e-3"} {
					converted, err := model.PreviewModelPricingConversion(name, model.PricingValues{
						"ModelPrice":      0.04,
						"ImagePriceTiers": ratio_setting.ImagePriceConfig{BaseSize: "1024x1024", BasePrice: 1, Tiers: []ratio_setting.ImagePriceTier{{Size: "256x256", Price: 0.4}, {Size: "1024x1792", Quality: "hd", Price: 3}}},
					})
					require.NoError(t, err)
					assert.NotEmpty(t, converted.UnsupportedReason, "pixel-tier matching must not be silently converted to different exact-size semantics")
...
func TestMetadataSyncLocaleAndEndpointValidation(t *testing.T) {
	for _, locale := range []struct{ input, expected string }{{"zh", "zh"}, {"zh-CN", "zh-CN"}, {"en", "en"}, {"ja", "ja"}} {
		...
		t.Run(dialect.kind, func(t *testing.T) {
			...
			for _, rule := range []int{model.NameRuleExact, model.NameRulePrefix, model.NameRuleContains, model.NameRuleSuffix} {
				t.Run(fmt.Sprintf("rule_%d_exact_names_only_atomic_and_cached", rule), func(t *testing.T) {
					...
					cached, err := model.GetRandomSatisfiedChannelWithFilters("default", name, 0, nil)
					require.NoError(t, err)
					...
					for _, group := range []string{"default", "vip", "last-model-group"} {
						cached, _ = model.GetRandomSatisfiedChannelWithFilters(group, name, 0, nil)
						assert.Nil(t, cached)
					}
					cached, err = model.GetRandomSatisfiedChannelWithFilters("default", name+"-keep", 0, nil)
					require.NoError(t, err)
					require.NotNil(t, cached)
