...
func TestLegacyTaskFallbackPreservesNativeRoutesWithoutClaimingWebPages(t *testing.T) {
	gin.SetMode(gin.TestMode)
	engine := gin.New()
	engine.NoRoute(legacyTaskRouteDispatcher(), func(c *gin.Context) { c.String(http.StatusOK, "web fallback") })
	for _, path := range []string{"/kling/v1/videos/text2video", "/jimeng/?Action=CVSync2AsyncSubmitTask"} {
		t.Run(path, func(t *testing.T) {
			response := httptest.NewRecorder()
			request := httptest.NewRequest(http.MethodPost, path, strings.NewReader(`{"model":"kling-v1","prompt":"test"}`))
			request.Header.Set("Content-Type", "application/json")
			engine.ServeHTTP(response, request)
			assert.NotEqual(t, http.StatusOK, response.Code)
			assert.NotContains(t, response.Body.String(), "web fallback")
		})
	}
	response := httptest.NewRecorder()
	engine.ServeHTTP(response, httptest.NewRequest(http.MethodGet, "/ordinary-page", nil))
	assert.Equal(t, http.StatusOK, response.Code)
	assert.Equal(t, "web fallback", response.Body.String())
}
