...
func PrepareRequestBilling(c *gin.Context, info *relaycommon.RelayInfo) *types.NewAPIError {
	...
	if imageRequest, ok := info.Request.(*dto.ImageRequest); ok && meta != nil {
		if ratio, configured := ratio_setting.GetImagePriceRatio(info.OriginModelName, ratio_setting.ImageRequestShape{Size: imageRequest.Size, Quality: imageRequest.Quality}); configured {
