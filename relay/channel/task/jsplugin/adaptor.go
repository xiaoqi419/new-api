...
func (a *TaskAdaptor) EstimateBillingValidated(c *gin.Context, info *relaycommon.RelayInfo) (map[string]float64, error) {
	...
	if ratio, configured := ratio_setting.GetImagePriceRatio(info.OriginModelName, ratio_setting.ImageRequestShape{Size: size, Quality: quality}); configured {
		if ratios == nil {
			ratios = make(map[string]float64)
		}
		ratios["image_tier"] = ratio
	}
	if a.plugin.Meta.Key == "doubao" {
		metadata, _ := request["metadata"].(map[string]any)
		if ratio, configured := doubao.VideoPriceRatio(info.OriginModelName, metadata); configured {
			if ratios == nil {
				ratios = make(map[string]float64)
			}
			delete(ratios, "video_input_ratio")
			ratios["video_tier"] = ratio
		}
	}
	return ratios, nil
}
...
func (a *TaskAdaptor) AdjustBillingOnSubmit(info *relaycommon.RelayInfo, taskData []byte) map[string]float64 {
	...
	// Submission hooks update measured quantities; configured host tiers remain
	// part of the original pricing contract across that adjustment.
	if len(ratios) > 0 && info != nil {
		for _, key := range []string{"image_tier", "video_tier"} {
			if ratio, exists := info.PriceData.OtherRatios()[key]; exists {
				ratios[key] = ratio
			}
		}
	}
