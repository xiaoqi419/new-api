...
func TestDoubaoPluginKeepsConfiguredVideoTiersAndSilentDefault(t *testing.T) {
	saved := ratio_setting.VideoPrice2JSONString()
	t.Cleanup(func() { require.NoError(t, ratio_setting.UpdateVideoPriceByJSONString(saved)) })
	require.NoError(t, ratio_setting.UpdateVideoPriceByJSONString(`{"doubao-seedance-1-5-pro-251215":{"base_price":8,"tiers":[{"has_audio":true,"price":24}]},"doubao-seedance-2-0-260128":{"base_price":46,"tiers":[{"has_video":true,"price":23}]}}`))
	source, err := plugins.Source("doubao")
	require.NoError(t, err)
	plugin, err := pluginruntime.NewRegistry().RegisterFactory(source, pluginruntime.Options{Key: "doubao"})
	require.NoError(t, err)
	for _, tc := range []struct {
		...
		{"silent default", "doubao-seedance-1-5-pro-251215", map[string]any{}, 1},
		{"explicit audio uses admin price", "doubao-seedance-1-5-pro-251215", map[string]any{"generate_audio": true}, 3},
		{"video admin price replaces plugin default", "doubao-seedance-2-0-260128", map[string]any{"content": []any{map[string]any{"type": "video_url", "video_url": map[string]any{"url": "https://example.com/video.mp4"}}}}, 0.5},
	} {
		t.Run(tc.name, func(t *testing.T) {
			adaptor := New(plugin)
			info := &relaycommon.RelayInfo{OriginModelName: tc.model, TaskRelayInfo: &relaycommon.TaskRelayInfo{}, ChannelMeta: &relaycommon.ChannelMeta{ChannelType: constant.ChannelTypeDoubaoVideo, UpstreamModelName: tc.model}}
			adaptor.Init(info)
			c, _ := gin.CreateTestContext(httptest.NewRecorder())
			c.Request = httptest.NewRequest(http.MethodPost, "/v1/videos", nil)
			c.Set("task_request", relaycommon.TaskSubmitReq{Model: tc.model, Metadata: tc.metadata})
			ratios, err := adaptor.EstimateBillingValidated(c, info)
			require.NoError(t, err)
			assert.Equal(t, map[string]float64{"video_tier": tc.want}, ratios)
		})
	}
}

func TestPluginImageTierSurvivesQuantityAdjustment(t *testing.T) {
	saved := ratio_setting.ImagePrice2JSONString()
	t.Cleanup(func() { require.NoError(t, ratio_setting.UpdateImagePriceByJSONString(saved)) })
	require.NoError(t, ratio_setting.UpdateImagePriceByJSONString(`{"mock-v1":{"base_size":"1K","base_price":1,"tiers":[{"size":"2K","price":3}]}}`))
	plugin, err := pluginruntime.NewRegistry().Register(mockPlugin, pluginruntime.Options{})
	require.NoError(t, err)
	adaptor := New(plugin)
	info := &relaycommon.RelayInfo{OriginModelName: "mock-v1", TaskRelayInfo: &relaycommon.TaskRelayInfo{}, ChannelMeta: &relaycommon.ChannelMeta{UpstreamModelName: "mock-v1"}}
	adaptor.Init(info)
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/images/generations", nil)
	c.Set("task_request", map[string]any{"model": "mock-v1", "size": "2K"})
	ratios, err := adaptor.EstimateBillingValidated(c, info)
	require.NoError(t, err)
	assert.Equal(t, 3.0, ratios["image_tier"])
	for key, value := range ratios {
		info.PriceData.AddOtherRatio(key, value)
	}
	adjusted := adaptor.AdjustBillingOnSubmit(info, []byte(`{"seconds":7}`))
	assert.Equal(t, 3.0, adjusted["image_tier"])
	assert.Equal(t, 7.0, adjusted["seconds"])
}
