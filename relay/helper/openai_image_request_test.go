...
func TestLegacyDalleValidationAndPricesRemainCompatible(t *testing.T) {
	...
	defaults, err := common.Marshal(ratio_setting.GetDefaultImagePriceCopy())
	require.NoError(t, err)
	require.NoError(t, ratio_setting.UpdateImagePriceByJSONString(string(defaults)))
	for _, tc := range []struct {
		...
		t.Run(tc.body, func(t *testing.T) {
			...
			assert.Equal(t, 1.0, meta.ImagePriceRatio, "DTO stays independent of host price settings")
			if ratio, configured := ratio_setting.GetImagePriceRatio(request.Model, ratio_setting.ImageRequestShape{Size: request.Size, Quality: request.Quality}); configured {
				meta.ImagePriceRatio = ratio
			}
			assert.Equal(t, tc.ratio, meta.ImagePriceRatio)
		})
	}
}
