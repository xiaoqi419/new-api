...
func TestGetTaskAdaptorPreservesLegacyPersistedPlatforms(t *testing.T) {
	...
	for _, platform := range platforms {
		_, isLegacy := GetTaskAdaptor(platform).(*legacyTaskBridge)
		assert.True(t, isLegacy, "persisted platform %s should retain legacy settlement", platform)
	}
}
