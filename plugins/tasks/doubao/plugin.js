...
// output is priced separately (Seedance 1.5 pro). This fork preserves its
// existing silent-output default by explicitly sending generate_audio=false. Seedance 1.0 lite is no longer listed and keeps the 1.0 pro tiers.
const VIDEO_MODELS = {
...
export function buildSubmitRequest(ctx) {
  ...
  if (videoProfile(ctx).audio && body.generate_audio === undefined) body.generate_audio = false;
  const imageContent = [];
...
export function extractUsage(ctx) {
  ...
  if (profile.audio) facts.generate_audio = metadata.generate_audio === true;
  return facts;
}
