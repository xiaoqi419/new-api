...
func TestDoubaoSeedanceUsageFacts(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"1.5 pro preserves silent default", pro15, map[string]any{"metadata": map[string]any{"resolution": "720p"}},
			map[string]any{"tokens": float64(108000), "resolution": "720p", "generate_audio": false}, ""},
		{"1.5 pro explicit audio", pro15, map[string]any{"metadata": map[string]any{"resolution": "720p", "generate_audio": true}},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.model == pro15 && metadata["generate_audio"] == nil {
