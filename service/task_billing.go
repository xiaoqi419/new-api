...
func RecalculateTaskQuotaByTokens(ctx context.Context, task *model.Task, totalTokens int) bool {
	...
	if snapshot := task.PrivateData.BillingContext; snapshot != nil {
		// A present context is the submission-time billing contract. Zero values
		// are free pricing, not permission to replace them with current settings.
		modelRatio, finalGroupRatio = snapshot.ModelRatio, snapshot.GroupRatio
	} else {
		// Pre-snapshot historical tasks have no frozen price to recover. Preserve
		// their legacy lookup using the task's original billing identity.
		modelName := taskModelName(task)
		var configured bool
