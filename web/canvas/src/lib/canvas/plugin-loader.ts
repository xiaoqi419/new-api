...
// 插件源码会通过动态 import 在本站域名下执行，等同于把页面权限交给该地址：
// 它能读到同源存储、并带着 cookie 以当前登录用户身份调用本站接口。
// 画布与主站同源发布，所以只放行随镜像发布的本站插件，任意外部地址一律拒绝。
// 若日后要放开，应改成显式白名单，而不是去掉这道检查。
function assertSameOriginPluginSource(url: string) {
    let resolved: URL;
    try {
    ...
    if (resolved.origin !== window.location.origin) throw new Error("出于安全考虑，只允许安装本站自带的插件");
}

async function fetchPluginSource(url: string) {
    assertSameOriginPluginSource(url);
