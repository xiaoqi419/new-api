...
/**
 * The host owns navigation in embedded mode, so the standalone video
 * workbench is intentionally unavailable there. Keep the tool names and a
 * stable result shape for older Agent clients, while leaving the full
 * workbench available to independent Canvas deployments.
 */
export const VIDEO_WORKBENCH_UNAVAILABLE = {
    ok: false,
    available: false,
    disabled: true,
    code: "video_workbench_unavailable",
    messageKey: "canvas.videoWorkbench.unavailable",
    message: "视频创作台当前不可用",
    note: "独立视频创作台已隐藏，画布内已有视频节点仍可打开和预览",
    navigated: "",
    run: false,
} as const;
...
function getVideoConfig() {
    if (isEmbedded()) return { ...VIDEO_WORKBENCH_UNAVAILABLE, operation: "get_config", applied: {} };
    const { config } = useConfigStore.getState();
