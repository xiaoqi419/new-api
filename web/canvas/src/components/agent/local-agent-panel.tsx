...
import { stripAgentConnectionParams, validateAgentEndpoint } from "@/lib/agent/agent-security";
...
import { acknowledgeCodexHistory, activateAgentClient, AgentApiError, discoverAgentConfig, fetchAgentJson, fetchAgentRequest, interruptCodexTurn, openAgentEventStream, postCodexApproval, postState, postToolResult } from "@/services/api/canvas-agent";
...
function clearAgentBootstrapParams() {
    if (typeof window === "undefined") return;
    const current = `${window.location.pathname}${window.location.search}${window.location.hash}`;
    const next = stripAgentConnectionParams(window.location.href);
    if (next && next !== current) window.history.replaceState(window.history.state, "", next);
}
...
export function LocalAgentPanel({ embedded, headless, autoConnect }: { embedded?: boolean; headless?: boolean; autoConnect?: boolean }) {
    ...
    useEffect(() => {
        ...
        const source = openAgentEventStream(endpoint, token, clientId);
    ...
    const toggleAgentConnection = async ({ silent = false }: { silent?: boolean } = {}) => {
        ...
        if (urlToken || urlEndpoint) clearAgentBootstrapParams();
        ...
        const endpointValidation = validateAgentEndpoint(nextEndpoint);
        ...
        errorLoggedRef.current = false;
        setAgentState({ url: endpointValidation.endpoint, token: nextToken, enabled: true, connected: false, silentConnect: silent, fragmentBootstrap: false, activity: rt("connecting"), connectError: "", activeTab: "setup" });
    };

    useLayoutEffect(() => {
        const bootstrap = readAgentUrlBootstrap(hash);
        if (searchParams.has("agentUrl") || searchParams.has("agentToken")) clearAgentBootstrapParams();
        if (!bootstrap) return;
        ...
        const endpointValidation = validateAgentEndpoint(bootstrap.url);
        if (!endpointValidation.ok) {
        ...
        errorLoggedRef.current = false;
        setAgentState({ url: endpointValidation.endpoint, token: bootstrap.token, enabled: true, connected: false, silentConnect: true, fragmentBootstrap: true, confirmTools: false, activity: rt("connecting"), connectError: "", activeTab: "setup" });
    }, [hash, navigate, searchParams, setAgentState]);
...
async function attachmentNodeOps(endpoint: string, token: string, clientId: string, value: unknown): Promise<CanvasAgentOp[]> {
    ...
    return await Promise.all(
        nodes.map(async (value) => {
            ...
            const res = await fetchAgentRequest(endpoint, token, `/agent/attachments/${encodeURIComponent(attachmentId)}?clientId=${encodeURIComponent(clientId)}`);
            if (!res.ok) {
    ...
    return await Promise.all(
        sources.map(async (source, index) => {
            const response = source.startsWith("data:image/")
                ? await fetch(source)
                : await fetchAgentRequest(endpoint, token, "/agent/local-image", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ path: source }) });
