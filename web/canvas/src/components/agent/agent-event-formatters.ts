...
import { isAgentMessageAsset, resolveAgentMessageAssetUrl } from "@/services/api/canvas-agent";
...
export function agentAttachmentToChatAttachment(item: AgentMessageAttachment, endpoint: string, token: string): AgentChatAttachment {
    ...
    return {
        id: item.id,
        name: item.name,
        url: resolveAgentMessageAssetUrl(endpoint, token, source),
        ...(isAgentMessageAsset(source) ? { assetRef: source } : {}),
    };
}
