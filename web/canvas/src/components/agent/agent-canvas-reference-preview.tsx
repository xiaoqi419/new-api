...
export function AgentCanvasReferencePreview({ reference, previewUrl, previewText, theme }: { reference: AgentCanvasReference; previewUrl?: string; previewText?: string; theme: (typeof canvasThemes)[keyof typeof canvasThemes] }) {
    ...
    const resolvedPreviewUrl = useAgentMessageAssetUrl(endpoint, token, previewUrl || "");
    ...
    return (
        <div className="w-64" style={{ color: theme.node.text }}>
            ...
            {reference.kind === "image" && resolvedPreviewUrl ? <img src={resolvedPreviewUrl} alt={reference.title} className="max-h-64 w-full rounded-md object-contain" /> : null}
            {reference.kind === "video" && resolvedPreviewUrl ? <video src={resolvedPreviewUrl} controls preload="metadata" className="max-h-64 w-full rounded-md" /> : null}
            {reference.kind === "audio" && resolvedPreviewUrl ? <audio src={resolvedPreviewUrl} controls preload="metadata" className="w-full" /> : null}
            ...
            {!resolvedPreviewUrl && !(reference.kind === "text" && previewText) ? (
