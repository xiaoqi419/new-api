                ...
                {attachments.length ? (
                    <div className="thin-scrollbar mb-2 flex gap-2 overflow-x-auto pb-1">
                        {attachments.map((item) => (
                            <AgentComposerAttachment key={item.id} item={item} theme={theme} onRemove={onRemoveAttachment} removeLabel={t("agent.composer.removeImage")} />
                        ))}
                    </div>
                ) : null}
...
function AgentComposerAttachment({ item, theme, onRemove, removeLabel }: { item: AgentChatAttachment; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; onRemove?: (id: string) => void; removeLabel: string }) {
    const endpoint = useAgentStore((state) => state.url);
    const token = useAgentStore((state) => state.token);
    const previewUrl = useAgentMessageAssetUrl(endpoint, token, item.assetRef || item.url);
    return (
        <div className="group relative size-14 shrink-0 overflow-hidden rounded-xl border" style={{ borderColor: theme.node.stroke }} title={item.name}>
            {previewUrl ? <img src={previewUrl} alt={item.name} className="size-full object-cover" /> : null}
            {onRemove ? (
                <button
                    type="button"
