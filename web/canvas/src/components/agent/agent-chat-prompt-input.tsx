...
import { isAgentMessageAsset } from "@/services/api/canvas-agent";
...
function ReferencePreview({ reference }: { reference: CanvasResourceReference }) {
    ...
    const previewUrl = useAgentMessageAssetUrl(endpoint, token, reference.previewUrl || "");
    if (reference.kind === "image" && previewUrl) return <img src={previewUrl} alt="" className="size-9 rounded-md object-cover" />;
...
function createReferenceToken(reference: AgentCanvasReference, theme: (typeof canvasThemes)[keyof typeof canvasThemes]) {
    ...
    if (reference.kind === "image" && reference.previewUrl && !isAgentMessageAsset(reference.previewUrl)) {
