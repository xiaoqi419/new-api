...
export const STATIC_I18N_KEYS = [
  ...
  // Wallet payment checkout (dynamic labels)
  'Base amount',
  'Total payment',
  'Dynamic network fee estimate',
  ...
  'Fee source: {{source}}',
  'Gateway quote',
  'Included in gateway amount',
  'Administrator fixed fee',
  'Administrator percentage fee',
  'Choose a payment currency',
  'Select a payment currency.',
  'Select a {{token}} payment network.',
  'Networks available: {{count}}',
  'Payment currency',
  'Payment failed',
  'Payment expired',
  'Personal Settings',
  ...
  // Misc
  // GMPay fee fallback configuration
  'GMPay fee fallback',
  ...
  'GMPay fee configuration must be valid JSON',
  'GMPay fee configuration must be a JSON object',
  'GMPay fee configuration contains an unknown field',
  'GMPay fee configuration version must be 1',
  'GMPay fee configuration enabled must be a boolean',
  'GMPay default fee must be a JSON object',
  'GMPay fee override must be a JSON object',
  'GMPay fee rule contains an unknown field',
  ...
  'GMPay fixed fee exceeds max_fee',
  'GMPay max_fee must be a non-negative decimal with up to 6 decimal places',
  'GMPay max_total must be a positive decimal with up to 6 decimal places',
  'GMPay fee overrides must be a JSON object',
  'GMPay fee override key must use TOKEN:network format',
  'GMPay fee override token must be USDT or USDC',
  'Additional price source URLs',
  ...
  'GMPay Solana representative message must be valid base64',
  'GMPay Solana transfer amount must be a non-negative integer in base units',
  'GMPay Solana transfer instruction must be transfer or transferChecked',
  ...
  'GMPay chain must be a JSON object',
  'GMPay chain contains an unknown field',
  'GMPay chain price_url or price_urls must be configured',
  'GMPay chain price_urls must be an array',
  'GMPay chain price_urls must contain between 1 and 8 sources',
  'GMPay chains contain a duplicate network',
  'GMPay chains contain an unsupported network',
  'GMPay chains must be a JSON object',
  'GMPay dynamic fee configuration requires at least one complete network reference pair',
  'GMPay dynamic fee configuration requires both RPC and price references for each configured network',
  'GMPay estimator mode is invalid',
  'GMPay fallback mode must match the default rule',
  'GMPay fallback_mode is required with fallback_value',
  'GMPay fallback_value must be a non-negative decimal with up to 6 decimal places',
  'GMPay fee override network is unsupported',
  'GMPay transaction context batch must be a boolean',
  'GMPay transaction context contains an unknown field',
  'GMPay transaction context integer fields are out of bounds',
  'GMPay transaction context must be a JSON object',
  'GMPay transaction context values are too long',
  'GMPay transaction contexts contain an unsupported network',
  'GMPay transaction contexts must be a JSON object',
  'Legacy Solana message (base64)',
