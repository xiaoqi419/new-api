...
it('keeps mapped model details available without response diagnostics', async () => {
  const user = userEvent.setup()
  render(<ModelBadge modelName='requested-model' actualModel='mapped-model' />)
  await user.click(
    screen.getByRole('button', { name: 'Model: requested-model' })
  )
  expect(await screen.findByText('mapped-model')).toBeVisible()
