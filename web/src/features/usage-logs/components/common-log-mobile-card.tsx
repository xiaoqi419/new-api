...
export function CommonLogMobileCard<TData>(props: {
  ...
  const cacheRead = log.cache_read_tokens ?? other?.cache_tokens ?? 0
  const cacheRate =
    inputTokens > 0 && cacheRead > 0
    ...
    <div className='min-w-0 space-y-2.5 text-sm leading-5'>
      ...
      {showTokens && (
        <div className='text-muted-foreground flex flex-wrap gap-x-3 gap-y-1 text-xs [overflow-wrap:anywhere]'>
          ...
          <span>
            {t('Total')}{' '}
            {(inputTokens + log.completion_tokens).toLocaleString()}
          </span>
          {cacheRead > 0 && (
            <span>
              {t('Cache')} ↓ {cacheRead.toLocaleString()}
              {cacheRate !== null
                ? ` (${cacheRate.toFixed(1).replace(/\.0$/, '')}%)`
                : ''}
            </span>
          )}
          {cacheWrite > 0 && (
