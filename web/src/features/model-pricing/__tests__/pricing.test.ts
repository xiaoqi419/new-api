...
describe('fork media tier pricing integration', () => {
  it('round-trips structured media tiers through options, editor draft and versioned save', () => {
    const video = {
      base_price: 46,
      tiers: [{ resolution: '1080p', has_audio: false, price: 51 }],
    }
    const image = {
      base_size: '1024x1024',
      base_price: 1,
    ...
    const options = pricingOptions({
      VideoPriceTiers: JSON.stringify({ movie: video }),
      ImagePriceTiers: JSON.stringify({ picture: image }),
    })
    const byModel = pricingValuesByModel(options)
    expect(
      pricingFromDraft(pricingRow('movie', byModel.get('movie') ?? {}))
        .VideoPriceTiers
    ).toEqual(video)
    expect(
      pricingFromDraft(pricingRow('picture', byModel.get('picture') ?? {}))
        .ImagePriceTiers
    ).toEqual(image)
    const changed = applyPricingDraft(options, {
      name: 'movie',
      billingMode: 'per-token',
      ratio: '23',
      videoPriceTiers: JSON.stringify(video),
    })
    expect(JSON.parse(changed.VideoPriceTiers).movie).toEqual(video)
    expect(JSON.parse(changed.ImagePriceTiers).picture).toEqual(image)
  })

  it('copies shared media tiers while retaining each target provider expression', () => {
    const video = { base_price: 1, tiers: [{ has_video: true, price: 2 }] }
    const options = pricingOptions({
      PluginBillingExpr: JSON.stringify({
        'plugin::target': 'tier("base", u("seconds") * 4)',
      }),
    })
    const changed = applyPricingDraft(
      options,
      {
        name: 'source',
        billingMode: 'per-token',
    ...
    expect(JSON.parse(changed.VideoPriceTiers).target).toEqual(video)
    expect(
      JSON.parse(changed['billing_setting.plugin_billing_expr'])[
        'plugin::target'
      ]
    ).toBe('tier("base", u("seconds") * 4)')
  })
})

it('does not erase local media tiers when syncing upstream scalar prices', () => {
  const image = {
    base_size: '1024x1024',
    base_price: 1,
    tiers: [{ quality: 'hd', price: 2 }],
  ...
  const next = applyPriceSyncSelections(options, {
    picture: { model_price: 0.04 },
  })
  expect(JSON.parse(next.ImagePriceTiers).picture).toEqual(image)
})

it('does not mark an unchanged media tier object as a dirty model', () => {
  const tiers = { base_price: 1, tiers: [{ resolution: '1080p', price: 2 }] }
  const before = pricingOptions({
    VideoPriceTiers: JSON.stringify({ movie: tiers }),
  })
  const snapshot: ModelPricingConfig = {
    options: before,
    entries: [],
  ...
  expect(buildPricingChanges(snapshot, before, { ...before })).toEqual([])
})
