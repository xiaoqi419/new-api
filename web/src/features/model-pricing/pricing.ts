...
> & {
  ...
  VideoPriceTiers?: Record<string, unknown>
  ImagePriceTiers?: Record<string, unknown>
}
...
export function pricingRows(options: PricingOptions): ModelPricingSnapshot[] {
  return buildModelSnapshots({
    ...
    videoPriceTiers: options.VideoPriceTiers,
    imagePriceTiers: options.ImagePriceTiers,
  })
}
...
export function pricingFromDraft(data: ModelRatioData): PricingValues {
  ...
  for (const [field, key] of [
    ['videoPriceTiers', 'VideoPriceTiers'],
    ['imagePriceTiers', 'ImagePriceTiers'],
  ] as const) {
    if (data[field]) {
      const tiers: unknown = JSON.parse(data[field])
      if (!tiers || typeof tiers !== 'object' || Array.isArray(tiers)) {
        throw new Error(t('Invalid pricing value'))
      }
      values[key] = tiers as Record<string, unknown>
    }
  }
...
export function pricingValuesByModel(
  ...
  for (const key of PRICING_KEYS) {
    ...
    for (const [name, value] of Object.entries(map)) {
      if (key === 'VideoPriceTiers' || key === 'ImagePriceTiers') {
        if (!value || typeof value !== 'object' || Array.isArray(value)) {
          throw new Error(t('Invalid pricing value'))
        }
        const model = models.get(name) ?? {}
        model[key] = value as Record<string, unknown>
...
export function applyPriceSyncSelections(
  ...
  for (const [name, fields] of Object.entries(selections)) {
    ...
    // Upstream scalar price sync has no media-tier contract. Retain local
    // resolution/quality and audio/video conditions unless explicitly edited.
    const existing = pricingValuesByModel(result).get(name)
    validated.VideoPriceTiers = existing?.VideoPriceTiers
    validated.ImagePriceTiers = existing?.ImagePriceTiers
