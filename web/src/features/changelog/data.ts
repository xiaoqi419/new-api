    ...
    changes: [
      {
        kind: 'feature',
        items: [
          '新增「淡雅浅粉 · 樱花」整套主题:近白粉底 + 柔玫瑰主色,并对按钮、输入框、标签页、下拉菜单、表格、进度条、侧边栏、滚动条等全组件做圆润 + 轻玻璃 + 柔投影的皮肤重塑,图标同步主题化(描边调细、侧栏导航图标加柔粉圆底「图标片」),含明暗两套。作为新增主题,不影响任何现有主题;用户可在「设置 → 外观 → 配色预设」随时切换,管理员可在「系统设置 → 运营 → 系统行为 → 默认主题」设置站点默认主题(用户可自行覆盖)。',
        ],
      },
      {
        kind: 'improvement',
