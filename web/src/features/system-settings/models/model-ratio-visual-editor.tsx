...
>(function ModelRatioVisualEditor(
  ...
  const handleDelete = useCallback(
    (name: string) => {
      ...
      const pluginMap = safeJsonParse<Record<string, string>>(
        pluginBillingExpr,
        { fallback: {}, silent: true }
      )
      for (const key of Object.keys(pluginMap)) {
        if (splitPluginBillingExprKey(key)?.[1] === name) delete pluginMap[key]
      }
      onChange('billing_setting.plugin_billing_expr', JSON.stringify(pluginMap))
