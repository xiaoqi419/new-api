...
>(function ModelPricingEditorPanel(
  ...
  const [videoPriceDraft, setVideoPriceDraft] = useState(
    createEmptyVideoPriceMatrix
  )
  const [imagePriceDraft, setImagePriceDraft] = useState(
    createEmptyImagePriceTiers
  )
  ...
  useEffect(() => {
    onDirtyChange?.(
      ...
      ) !== canonicalizeVideoPriceTiers(editData?.videoPriceTiers) ||
        canonicalizeImagePriceTiers(
          serializeImagePriceTiers(imagePriceDraft, watchedValues.price)
        ) !== canonicalizeImagePriceTiers(editData?.imagePriceTiers) ||
  ...
  const videoErrorKey =
    pricingMode === 'per-token'
      ? getVideoPriceMatrixError(videoPriceDraft, promptPrice)
      : null
  const imageErrorKey =
    pricingMode === 'per-request'
      ? getImagePriceTiersError(imagePriceDraft, watchedValues.price)
      : null
  const videoPriceError = videoErrorKey ? t(videoErrorKey) : null
  const imagePriceError = imageErrorKey ? t(imageErrorKey) : null
  ...
  const validatePricingValues = useCallback(() => {
    ...
    return !videoPriceError && !imagePriceError
  }, [
    form,
    laneEnabled,
    lanePrices,
