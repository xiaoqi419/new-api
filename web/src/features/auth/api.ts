...
export async function createOAuthAuthorization(
  ...
  operation?: VerificationOperation | CaptchaQuery,
  signal?: AbortSignal,
  proofToken?: string
): Promise<{ state: string; authorizationUrl?: string }> {
  const captcha = operation && !('scope' in operation) ? operation : undefined
  const verification = operation && 'scope' in operation ? operation : undefined
