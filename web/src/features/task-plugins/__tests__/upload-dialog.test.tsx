...
describe('UploadDialog layout', () => {
  test('renders the source editor and hides the native file input from view', async () => {
    renderDialog()

    await waitFor(() => expect(sourceEditor()).toBeInTheDocument())
  ...
  test('does not steal focus into the mid-dialog source editor on open', async () => {
    renderDialog()

    await screen.findByRole('textbox', { name: 'Plugin source' })
