...
it('sends balance sorting to the server and preserves invitation badges', async () => {
...
it('preserves independently visible creation and last login columns', async () => {
  ...
  expect(
    within(row).getByText(
      formatTimestamp(Math.floor(Date.now() / 1000) - 86400)
    )
  ).toBeInTheDocument()
  expect(
    within(row).getByText(formatTimestamp(Math.floor(Date.now() / 1000) - 20))
  ).toBeInTheDocument()
  expect(within(row).getByText('No Inviter')).toBeInTheDocument()
  await userEvent.click(screen.getByRole('button', { name: 'View' }))
  await userEvent.click(
    screen.getByRole('menuitemcheckbox', { name: 'Last Login' })
  )
  expect(
