...
export async function requestClassicWalletEpayCheckout({
  api,
  topUpCount,
  paymentMethod,
  money,
  ...
  const response = await api.post('/api/user/epay/checkout', {
...
const TopUp = () => {
  ...
  const [wechatPayNative, setWechatPayNative] = useState(true)
  const [wechatPayH5, setWechatPayH5] = useState(false)
  const [wechatPayJSAPI, setWechatPayJSAPI] = useState(false)
  ...
  const onlineTopUp = async () => {
    ...
    if (payWay === 'wechatpay') {
      ...
      setConfirmLoading(true)
      try {
        // 微信内置浏览器：优先 JSAPI（需服务号网页授权）
        if (inWeChat && wechatPayJSAPI) {
          const res = await API.post('/api/user/wechatpay/jsapi/prepare', {
            amount: parseInt(topUpCount),
          })
          const { message, data } = res.data
          if (message === 'success' && data?.authorize_url) {
        ...
        // 手机外部浏览器：H5 唤起微信
        if (isMobile && !inWeChat && wechatPayH5) {
          ...
          const { message, data } = res.data
          if (message === 'success' && data?.h5_url) {
            if (isSafeHttpCheckoutUrl(data.h5_url)) {
    ...
    try {
      const isEpay =
        payMethods.some((method) => method.type === payWay) &&
        payWay !== 'alipay_direct' &&
        payWay !== 'wechatpay' &&
        payWay !== 'creem' &&
        !payWay.startsWith('waffo')
      if (isEpay) {
        epayRetryRef.current = { topUpCount, payWay, amount }
    ...
    try {
      ...
      if (success) {
        ...
        try {
          ...
          setWechatPayNative(data.wechatpay_native || false)
          setWechatPayH5(data.wechatpay_h5 || false)
          setWechatPayJSAPI(data.wechatpay_jsapi || false)
    ...
    <div className='w-full max-w-7xl mx-auto relative min-h-screen lg:min-h-0 mt-[60px] px-2'>
      ...
      <EpayCheckoutModal
        ...
        onRetry={async () => {
          ...
          try {
            const response = await API.post('/api/user/epay/checkout', {
              amount: parseInt(retry.topUpCount),
              payment_method: retry.payWay,
            })
            const checkout = normalizeEpayCheckout(response.data?.data, {
              paymentMethod: retry.payWay,
              ...(retry.amount > 0 ? { money: retry.amount } : {}),
            })
            if (response.data?.message !== 'success' || !checkout) {
              showError(t('支付请求失败'))
              return
            }
            openClassicWalletEpay(
