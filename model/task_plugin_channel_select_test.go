...
func TestTaskPluginChannelSelectionFiltersBothCachePaths(t *testing.T) {
	...
	selected, err := GetChannelWithFilters("default", "shared", 0, identityFilters("alpha", nil))
	require.NoError(t, err)
	require.NotNil(t, selected)
	assert.Equal(t, "alpha", selected.Name)
	selected, err = GetChannelWithFilters("default", "shared", 0, identityFilters("", nil))
	require.NoError(t, err)
	assert.Nil(t, selected)
	selected, err = GetChannelWithFilters("default", "ordinary", 0, identityFilters("", nil))
	require.NoError(t, err)
	require.NotNil(t, selected)
	assert.Equal(t, "ordinary", selected.Name)
	selected, err = GetChannelWithFilters("default", "legacy", 0, identityFilters("legacy-alpha", []int{constant.ChannelTypeKling}))
	require.NoError(t, err)
	require.NotNil(t, selected)
	assert.Equal(t, "legacy-alpha", selected.Name)
	selected, err = GetChannelWithFilters("default", "legacy", 0, identityFilters("legacy-alpha", []int{constant.ChannelTypeKling, constant.ChannelTypeJimeng}))
	require.NoError(t, err)
	require.NotNil(t, selected)
...
func TestNewAPIChannelServesExtendedTaskPlugins(t *testing.T) {
	...
	selected, err = GetChannelWithFilters("default", "shared", 0, identityFilters("delta", nil))
	require.NoError(t, err)
	require.NotNil(t, selected)
	assert.Equal(t, "gateway-single", selected.Name, "a single task_plugin_key still binds a New API channel")
	selected, err = GetChannelWithFilters("default", "shared", 0, identityFilters("beta", nil))
	require.NoError(t, err)
	...
	selected, err = GetChannelWithFilters("default", "shared", 0, shared)
	require.NoError(t, err)
	require.NotNil(t, selected)
	assert.Equal(t, "gateway", selected.Name, "a bound shared-model candidate admits the channel")
	selected, err = GetChannelWithFilters("default", "chat", 0, identityFilters("", nil))
	require.NoError(t, err)
	require.NotNil(t, selected)
	assert.Equal(t, "gateway", selected.Name, "requests without a pinned plugin keep using the gateway")
}

func TestTaskPluginFiltersPreserveRoutingPoolAllowlistAndExclusions(t *testing.T) {
	db := useChannelFailoverOptionDB(t)
	require.NoError(t, db.AutoMigrate(&Ability{}))
	originalCache := common.MemoryCacheEnabled
	t.Cleanup(func() { common.MemoryCacheEnabled = originalCache })
	alpha := `{"task_plugin_key":"alpha"}`
	beta := `{"task_plugin_key":"beta"}`
	channels := []Channel{
		{Type: constant.ChannelTypeTaskPlugin, Status: common.ChannelStatusEnabled, Name: "alpha", Group: "default", Models: "shared", Setting: &alpha},
		{Type: constant.ChannelTypeTaskPlugin, Status: common.ChannelStatusEnabled, Name: "beta", Group: "default", Models: "shared", Setting: &beta},
	}
	for i := range channels {
		require.NoError(t, channels[i].Insert())
	}
	for _, cached := range []bool{false, true} {
		...
		InitChannelCache()
		filter := ChannelSelectionFilter{Filters: identityFilters("alpha", nil), AllowedChannelIDs: map[int]struct{}{channels[1].Id: {}}}
		selected, err := GetRandomSatisfiedChannelFiltered("default", "shared", 0, "", filter)
		require.NoError(t, err)
		assert.Nil(t, selected, "plugin identity must not widen a routing pool")
		filter.AllowedChannelIDs[channels[0].Id] = struct{}{}
		selected, err = GetRandomSatisfiedChannelFiltered("default", "shared", 0, "", filter)
		require.NoError(t, err)
		require.NotNil(t, selected)
		assert.Equal(t, channels[0].Id, selected.Id)
		filter.ExcludedChannelIDs = map[int]struct{}{channels[0].Id: {}}
		selected, err = GetRandomSatisfiedChannelFiltered("default", "shared", 0, "", filter)
		require.NoError(t, err)
		assert.Nil(t, selected, "excluded channels must not return through plugin routing")
	}
}
