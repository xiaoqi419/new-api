...
func RecordAuditLog(c *gin.Context, entry AuditLog) {
	if err := RecordAuditLogWithError(c, entry); err != nil {
		logger.LogError(c, err.Error())
	}
}

func RecordAuditLogWithError(c *gin.Context, entry AuditLog) error {
	...
	if LOG_DB == nil {
		return fmt.Errorf("audit log write failed (request_id=%s): log database unavailable", entry.RequestId)
	}
	var row any = &entry
	if common.UsingLogDatabase(common.DatabaseTypeClickHouse) {
		encoded, err := common.Marshal(entry.Other)
		if err != nil {
			return fmt.Errorf("audit log write failed (request_id=%s): %w", entry.RequestId, err)
		}
