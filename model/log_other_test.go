...
func TestLogOtherScopesAndMerges(t *testing.T) {
	...
	other.MergePublic(map[string]any{"zero": 0})
	assert.True(t, other.SetAdmin("use_channel", []string{"channel-a"}))
	other.MergeAdmin(map[string]any{"rejected": false})
	assert.True(t, other.SetRoot("upstream_request_id", "upstream-private"))
	other.MergeRoot(map[string]any{"generation": 0})
	assert.True(t, other.SetAudit("method", "POST"))
	other.MergeAudit(map[string]any{"success": false})

	require.JSONEq(t, `{
        "request_path": "/v1/chat/completions",
        "zero": 0,
        "admin_info": {"use_channel": ["channel-a"], "rejected": false},
        "root_info": {"upstream_request_id": "upstream-private", "generation": 0},
        "audit_info": {"method": "POST", "success": false}
    }`, other.JSONString())
}
...
func TestLogOtherRejectsSensitivePublicFields(t *testing.T) {
	...
	for _, key := range []string{
		"admin_info", "root_info", "audit_info", "channel_id", "channel_name", "channel_type", "reject_reason",
	} {
...
func TestLogOtherFromMapScopesLegacyValues(t *testing.T) {
	other := logOtherFromMap(map[string]any{
		"request_path": "/v1/chat/completions",
		"channel_id":   42,
		"admin_info":   map[string]any{"use_channel": []string{"a"}},
		"root_info":    map[string]any{"debug": true},
		"audit_info":   map[string]any{"method": "POST"},
	})

	require.JSONEq(t, `{
        "request_path": "/v1/chat/completions",
        "admin_info": {"channel_id": 42, "use_channel": ["a"]},
        "root_info": {"debug": true},
