...
func TestRedeemSupportsWalletQuotaAboveInt32(t *testing.T) {
	userId, _ := setupRedeemFixture(t, 500)
	const largeQuota = common.MaxQuota + 1234
	redemption := &Redemption{
	...
	quota, err := Redeem(redemption.Key, userId)
	require.NoError(t, err)
	assert.Equal(t, largeQuota, quota)

	var user User
	require.NoError(t, DB.First(&user, "id = ?", userId).Error)
	assert.Equal(t, largeQuota, user.Quota)
}
