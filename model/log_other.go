...
// logOtherFromMap converts legacy map-based log metadata into the scoped
// representation.  A number of provider and task call sites still assemble
// their metadata as map[string]any; routing the conversion here keeps those
// callers source-compatible while ensuring privileged keys cannot be written
// at the public top level.
func logOtherFromMap(values map[string]any) *LogOther {
	other := NewLogOther()
	for key, value := range values {
		switch key {
		case logOtherAdminInfoKey:
			if nested, ok := value.(map[string]any); ok {
				other.MergeAdmin(nested)
				continue
			}
			other.SetAdmin(key, value)
		case logOtherRootInfoKey:
			if nested, ok := value.(map[string]any); ok {
				other.MergeRoot(nested)
				continue
			}
			other.SetRoot(key, value)
		case logOtherAuditInfoKey:
			if nested, ok := value.(map[string]any); ok {
				other.MergeAudit(nested)
				continue
			}
			other.SetAudit(key, value)
		default:
			if slices.Contains(legacySensitiveLogOtherKeys, key) {
				other.SetAdmin(key, value)
			} else {
...
// asLogOther accepts both the current scoped type and legacy map metadata.
// Keeping this boundary permissive lets independently updated provider
// adapters continue to compile while all persisted output uses LogOther.
func asLogOther(value any) *LogOther {
	...
	case LogOther:
		...
		return &copy
	case map[string]any:
		return logOtherFromMap(typed)
	default:
		return NewLogOther()
	}
}

func asLogOtherMap(value any) map[string]any {
	switch typed := value.(type) {
	case map[string]any:
		return typed
	case *LogOther:
