	...
	for _, key := range []string{"ImagePriceTiers", "VideoPriceTiers"} {
		if value, exists := effective[key]; exists {
			...
			if len(config.Tiers) > 0 {
				return &ModelPricingConversion{UnsupportedReason: "Media tier pricing must be converted manually to preserve all resolution, quality, and audio/video conditions."}, nil
			}
		}
	}
