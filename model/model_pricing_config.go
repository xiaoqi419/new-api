...
var modelPricingOptionKeys = []string{
	...
	"CreateCacheRatio", "ImageRatio", "ImagePriceTiers", "ModelPrice", "ModelRatio", "VideoPriceTiers",
...
func defaultPricingMaps() map[string]map[string]any {
	...
	for name, cfg := range ratio_setting.GetDefaultImagePriceCopy() {
		encoded, _ := common.Marshal(cfg)
		var value map[string]any
		_ = common.Unmarshal(encoded, &value)
		result["ImagePriceTiers"][name] = value
	}
	for name, cfg := range ratio_setting.GetDefaultVideoPriceCopy() {
		encoded, _ := common.Marshal(cfg)
		var value map[string]any
		_ = common.Unmarshal(encoded, &value)
		result["VideoPriceTiers"][name] = value
	}
	return result
}
...
func readModelPricingMaps(db *gorm.DB) (map[string]map[string]any, map[string]bool, []string, error) {
	...
	for _, row := range rows {
		...
		if existing[row.Key] && ModelPricingVersion(PricingValues(values[row.Key])) != ModelPricingVersion(PricingValues(entries)) {
			return nil, nil, nil, fmt.Errorf("options key %q has conflicting duplicate pricing values; reconcile the duplicate rows before changing pricing", row.Key)
		}
...
func validateModelPricing(name string, values, previous PricingValues) error {
	...
	for key, value := range values {
		...
		if key == "ImagePriceTiers" || key == "VideoPriceTiers" {
			encoded, err := common.Marshal(value)
			if err != nil {
				return fmt.Errorf("%s: %w", key, err)
			}
			if string(encoded) == "null" {
				return fmt.Errorf("%s must be a tier configuration object", key)
			}
			if key == "ImagePriceTiers" {
				var config ratio_setting.ImagePriceConfig
				if err := common.Unmarshal(encoded, &config); err != nil {
					return fmt.Errorf("%s: %w", key, err)
				}
				if err := ratio_setting.ValidateImagePriceConfig(name, config); err != nil {
					return err
				}
			} else {
				var config ratio_setting.VideoPriceConfig
				if err := common.Unmarshal(encoded, &config); err != nil {
					return fmt.Errorf("%s: %w", key, err)
				}
				if err := ratio_setting.ValidateVideoPriceConfig(name, config); err != nil {
					return err
				}
			}
			continue
