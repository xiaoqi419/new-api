...
func InitPasswordEncryption() error {
	...
	if DB.Migrator().HasTable(&Option{}) {
		err := DB.Where(Option{Key: passwordEncryptionOptionKey}).First(&legacy).Error
		if err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
			return fmt.Errorf("read legacy password encryption key: %w", err)
		}
		privateKeyPEM = legacy.Value
	}
	if privateKeyPEM == "" {
		var err error
	...
	} else if err := common.LoadPasswordEncryptionPrivateKey(privateKeyPEM); err != nil {
		return fmt.Errorf("load legacy password encryption key: %w", err)
	}
