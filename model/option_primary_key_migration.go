	...
	for _, row := range rows {
		...
		if i, ok := seen[row.Key]; ok {
			if deduped[i].Value != row.Value {
				return nil, 0, fmt.Errorf("options key %q has conflicting duplicate values; back up options and reconcile this key to one intended value before restarting; no rows were changed", row.Key)
			}
