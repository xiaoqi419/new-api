...
- Keep only the two long-lived local branches `main` (release) and `secondary-dev` (development). A feature branch/worktree is temporary, must be based on the current `origin/main`, and must be removed after its verified commit is merged. Never create a second implementation worktree for the same requirement.
