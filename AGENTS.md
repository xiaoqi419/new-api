...
- Whenever you ship a user-facing change (new feature, enhancement, or fix), you MUST add a corresponding entry to the changelog at `web/src/features/changelog/data.ts` (newest first, with `version` matching the image tag baked by `build-push.sh`, e.g. `YYYYMMDD-<sha>`).
...
- This repository keeps exactly one active Native Comet change at a time. Before starting another user-visible requirement, the current change must be completed and archived, or explicitly abandoned with a recoverable external backup; a stale or workspace-mismatched change must be repaired or removed before new work begins.
- Keep `main` as the only long-lived local branch and keep its checkout clean. Develop in temporary feature branches/worktrees based on the current `origin/main`, then remove them after their verified commits are merged. `secondary-dev` is retired and must not be reused as a development baseline. Never create a second implementation worktree for the same requirement.
- Before retiring a legacy checkout, review and preserve unique commits, uncommitted source, and unresolved drafts in a verified external recovery archive. Never recursively clean independent repositories, runtime databases, logs, or user assets as development residue.
